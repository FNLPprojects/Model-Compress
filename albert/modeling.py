    ...
    with tf.variable_scope(scope, default_name="bert"):
      ...
      with tf.variable_scope("pooler"):
        ...
        self.pooled_output = tf.layers.dense(
            ...
            config.hidden_size,
            activation=tf.tanh,
            kernel_initializer=create_initializer(config.initializer_range))#接一个全连接层进行输出 [batch_size, hidden_size]
...
def transformer_model(input_tensor,
  ...
  if input_width != hidden_size:
    prev_output = dense_layer_2d(
        ...
        None, use_einsum=use_einsum, name="embedding_hidden_mapping_in")#增加了embedding_hidden_mapping_in层，即增加了E*H的矩阵。使得Embedding层和Encoder层解除绑定
  else:
    ...
    for layer_idx in range(num_hidden_layers):
      ...
      with tf.variable_scope("group_%d" % group_idx):
        with tf.name_scope("layer_%d" % layer_idx):
          layer_output = prev_output#之前的给输出给layer_output
